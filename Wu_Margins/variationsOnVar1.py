...
def generateInitialData(model: AgentModel):
    initial_data = {
        ...
        "b_success_rate": random.uniform(0.5, 0.99),
        # Wu's models aways set B as strictly better than A, plus some epsilon.
        # How does success rate here differ from objective_b
        # Might want something like "b_success_rate": random.uniform(0.5, 0.99)
        # Previously: "b_success_rate": random.uniform(0.01, 0.99)
        # We don't want B to give unsuccessful trials more frequently even when the objective probability is higher
